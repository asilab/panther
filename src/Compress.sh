...
function COMPRESS_NORM(){

rm -f ../reports/REPORT_COMPLEXITY_NC_NORMAL_$1;
cd $1
for x in *.jpg.pgm.PROCESSED.bin;
    do
    echo "Running $x ... in $1...";
      ...
      compressed=`ls -la $x.paq8kx | awk '{ print $5;}'`;
      entropy=`echo "scale=10; ($compressed * 8.0) / $original" | bc -l | awk '{printf "%f", $0}'`;
      echo "$x : $entropy" >> ../../reports/REPORT_COMPLEXITY_NC_NORMAL_$1;
  done
cd ../
}
...
COMPRESS_NORM "normalize_Quantizing8" &
P=$!
wait $P
cd ../
#
